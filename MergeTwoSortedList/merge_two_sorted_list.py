...
def mergeTwoLists(list1: Optional[ListNode], list2: Optional[ListNode]) -> Optional[ListNode]:
    ...
    elements = add_element_to_list(list1, elements)
    elements = add_element_to_list(list2, elements)
    elements.sort()

    result = createListNode(elements)
    print(result)

    printAllNodeValue(result)

    
def add_element_to_list(node: Optional[ListNode], result: List[int]) -> List[int]:
    current_node = node
...
def createListNode(sortedList: List[int]) -> Optional[ListNode]:
    result = []
    if len(sortedList) == 0:
        return []
...
def printAllNodeValue(node: ListNode):
    current_node = node
    if current_node == []:
        return current_node
    while True:
